            ...
            <DrawerSection title={t.bookings.revenueBreakdown}>
              <dl className="divide-y divide-hairline">
                {/* The guest price is VAT-inclusive, so the split is shown in full:
                    ...
                    commission — which is charged on the base, never on the gross. Every
                    figure renders the API's frozen value exactly as sent (authoritative
                    since 2026-08-29); recomputing any of them here can disagree with
                    the stored booking. See the contract note on the Booking type. */}
                <DrawerStatRow label={t.bookings.grossWithVat} value={formatSAR(detail.total)} />
                <DrawerStatRow label={t.bookings.netBase} value={formatSAR(detail.netBase)} />
                <DrawerStatRow
